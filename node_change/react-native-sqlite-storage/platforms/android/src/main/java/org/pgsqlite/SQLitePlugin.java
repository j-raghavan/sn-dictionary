...
public class SQLitePlugin extends ReactContextBaseJavaModule {
     ...
     * @throws Exception
     */
    private SQLiteDatabase openDatabase(String dbname,String location, String assetFilePath, int openFlags, CallbackContext cbc) throws Exception {
        InputStream in = null;
        File dbfile = null;
        try {
            ...
            if (dbfile == null) {
                ...
                if (TextUtils.isEmpty(location)) {
                    ...
                    dbfile = new File(this.getContext().getFilesDir() + File.separator + location + dbname);
                }
//                dbfile = this.getContext().getDatabasePath(dbname);
    ...
    private void deleteDatabase(String dbname, CallbackContext cbc) {
        ...
        if (r != null) {
            ...
            boolean deleteResult = this.deleteDatabaseNow(dbname, null);
            if (deleteResult) {
     ...
     */
    @SuppressLint("NewApi")
    private boolean deleteDatabaseNow(String dbname, String location) {
        File dbfile;
        if(TextUtils.isEmpty(location)) {
            dbfile = this.getContext().getDatabasePath(dbname);
        } else {
    ...
    private class DBRunner implements Runnable {
        ...
        DBRunner(final String dbname, ReadableMap options, CallbackContext cbc) {
            ...
            location = SQLitePluginConverter.getString(options,"location",null);
            Log.i(TAG,"DBRunner dblocation:"+location);
            Log.i(TAG,"DBRunner options:"+options.toString());
        ...
        public void run() {
            try {
                this.mydb = openDatabase(dbname,location, this.assetFilename, this.openFlags, this.openCbc);
            } catch (SQLiteException ex) {
            ...
            try {
                ...
                while (!dbq.stop) {
                    ...
                    if (androidLockWorkaround && dbq.queries.length == 1 && dbq.queries[0].equals("COMMIT")) {
                        ...
                        this.mydb = openDatabase(dbname, location,"", this.openFlags, null);
            ...
            if (dbq != null && dbq.close) {
                try {
                    ...
                    if (!dbq.delete) {
                        ...
                        try {
                            boolean deleteResult = deleteDatabaseNow(dbname, location);
                            if (deleteResult) {
                                dbq.cbc.success("database removed");
                            } else {
