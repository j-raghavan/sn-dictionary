...
newSQLError = function(error, code) {
  ...
  if (!sqlError) {
    sqlError = new Error("a plugin_sticker.snplg had an error but provided no response");
    sqlError.code = code;
  }
...
SQLitePlugin = function(openargs, openSuccess, openError) {
  ...
  if (typeof dbname !== "string") {
    throw newSQLError("sqlite plugin_sticker.snplg database name must be a string");
  }
...
SQLitePluginTransaction.prototype.handleStatementSuccess = function(handler, response) {
  // plugin_sticker.snplg.log("handler response:",response,response.rows);
  var payload, rows;
  if (!handler) {
    return;
  }
  rows = response.rows || [];
  // plugin_sticker.snplg.log("handler rows now:",rows);
  payload = {
    rows: {
      item: function(i) {
        return rows[i];
  ...
  // plugin_sticker.snplg.log("handler response payload:",payload);
